...
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
uint8_t buffer[10]; // obszar pamieci o ulotnej zawartosci
int flaga = 0;
int end_message = 0;
/* USER CODE END PM */
...
void read_message()
{
	for (int i = 0; i < sizeof(buffer); i++)
	{
		switch(buffer[i])
		{
		case '[': // poczatek wiadomosci
			for(int j = i; j < sizeof(buffer); j++) //
			{
				if(buffer[j] == ']')
				{
					data_for_display[j-i-1] = ' ';
					end_message = 1;
					break;
				}
				else
				{
					data_for_display[j-i] = buffer[j+1];
				}
			}
			break;
		}
